...
const staticLogins = [
 { d: "Mon", success: 4200, failed: 120 },
 { d: "Tue", success: 4800, failed: 142 },
 { d: "Wed", success: 5100, failed: 98 },
 { d: "Thu", success: 4900, failed: 110 },
 { d: "Fri", success: 6200, failed: 180 },
 { d: "Sat", success: 7100, failed: 220 },
 { d: "Sun", success: 6800, failed: 190 },
];

type ViewMode = "line" | "area";

export default function SecurityPage() {
 const [logins, setLogins] = useState(staticLogins);
 ...
 // merge backend data with static extended data
 const merged = staticLogins.map((row) => {
 const found = res.loginActivity.find((r) => r.day === row.d.substring(0, 3));
 return found ? { ...row, success: found.success, failed: found.failed } : row;
 });
 setLogins(merged);
 } catch {
 setError("Backend data unavailable — showing cached data.");
 } finally {
 setLoading(false);
 }
 };
 ...
 const avgSessions = Math.round(totalSuccess / logins.length);
 ...
 <StatCard label="Blocked Accounts" value="312" icon={Ban} tone="violet" valueClassName="text-xl" />
 </div>
 ...
 >
 <span className="h-[0.556vw] w-[0.556vw] rounded-full bg-blue-500" /> Successful
 </button>
 <button
 ...
 >
 <span className="h-[0.556vw] w-[0.556vw] rounded-full bg-rose-400" /> Failed
 </button>
 </div>
 </div>
