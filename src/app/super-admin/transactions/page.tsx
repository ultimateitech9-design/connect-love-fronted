...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    ...
    managementFetch<Row[]>('/users/admin/coin-transactions')
      .then(setRows)
  ...
  const updateWithdrawal = async (row: Row, status: 'completed' | 'rejected') => {
    ...
    setActionId(row.id);
    try {
      await managementFetch(`/users/admin/coin-transactions/${row.id}/withdrawal`, { method: 'PATCH', body: JSON.stringify({ status }) });
